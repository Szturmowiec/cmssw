...
cal=$(realpath $3)
cd $2$"/src/Analyzer/DiamondTimingAnalyzer/test"
if [ $? -ne 0 ]; then
...
do
  ...
  if [ $i -eq 0 ]
  then
    cmsRun Analyzer/DiamondTimingAnalyzer/test/cfg.py calibFile=$cal geometryFile=$4 validOOT=$5 debug=$6 outputFile="out.root"
    if [ $? -ne 0 ]; then
    ...
    cmsRun Analyzer/DiamondTimingAnalyzer/test/cfg.py calibFile="Analyzer/DiamondTimingAnalyzer/test/"$f geometryFile=$4 validOOT=$5 debug=$6 outputFile="out.root"
    if [ $? -ne 0 ]; then
        echo "Error while executing the cmsRun command."
        exit 2
    fi
  ...
  root -b -q $json_producer$'++("'$src_dir$'/out.root",'$l',"'$src_dir$'/Analyzer/DiamondTimingAnalyzer/test/Calibration_L'$i$'.json",'0')'
  if [ $? -ne 0 ]; then
...
output=$(date "+%F_%T")
python3 $plot Analyzer/DiamondTimingAnalyzer/test $output
if [ $? -ne 0 ]; then
